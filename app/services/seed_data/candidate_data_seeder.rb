  ...
  class CandidateDataSeeder < BaseService
    ...
    def seed_candidate_data
      ...
      faker_count.times do |i|
        ...
        Rails.logger.debug { "   - Skills: #{user.candidate.skills.count}" }
        Rails.logger.debug { "   - Experiences: #{user.candidate.experiences.count}" }
        Rails.logger.debug { "   - Role Types: #{user.candidate.role_type.present?}" }
        Rails.logger.debug { "   - Role Levels: #{user.candidate.role_level.present?}" }
        Rails.logger.debug { "   - Bio: #{user.candidate.bio.present? ? 'Present' : 'Missing'}" }
        Rails.logger.debug { "   - Social Links: #{user.candidate.social_link.present? ? 'Present' : 'Missing'}" }
        Rails.logger.debug { "   - Location: #{user.candidate.location.present? ? 'Present' : 'Missing'}" }
      rescue StandardError => e
        Rails.logger.debug { "⚠️  Failed to create candidate #{i + 1}: #{e.message}" }
    ...
    def create_user(index)
      ...
      unless user
        ...
        if user.save(validate: false)
          Rails.logger.debug { "Candidate User created with email #{user.email}" }
          # Attach avatar from RandomUser.me API in a separate transaction
          begin
            attach_random_user_avatar(user, user_data[:avatar_url])
          rescue StandardError => e
            Rails.logger.debug { "⚠️  Avatar attachment failed for #{user.email}: #{e.message}" }
            # Continue without avatar - user is still created successfully
          end
        else
          Rails.logger.debug { "Candidate User Failed to create #{user.errors.full_messages}" }
        end
      end
    ...
    def create_candidate(user)
      ...
      RoleType::TYPES.sample(rand(1..3))
      RoleLevel::TYPES.sample(rand(1..3))
    ...
    def fetch_random_user_data
      ...
      Rails.logger.debug { "⚠️  Failed to fetch from RandomUser.me API: #{e.message}" }
      generate_fallback_user_data
    end
