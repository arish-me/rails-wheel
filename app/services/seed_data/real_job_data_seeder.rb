...
module SeedData
  class RealJobDataSeeder < BaseService
    ...
    def seed_real_job_data
      Rails.logger.debug { "🌐 Seeding real job data for #{company.name}..." }
      ...
      faker_count.times do |i|
        ...
        begin
          ...
          Rails.logger.debug { "⚠️  Failed to create job: #{e.message}" }
          next
        end

        # Set external data for some jobs to simulate external integrations
        add_external_data(job) if rand < EXTERNAL_DATA_CHANCE
      end

      Rails.logger.debug { "✅ Created #{created_count} real job listings for #{company.name}" }
    end
    ...
    def add_external_data(job)
      external_sources = %w[linkedin indeed glassdoor ziprecruiter]
      ...
      Rails.logger.debug { "⚠️  Failed to add external data for job #{job.id}: #{e.message}" }
    end
    ...
    def generate_salary_variation(base_min, base_max)
      min_variation = rand(-10_000..10_000)
