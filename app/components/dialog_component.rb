class DialogComponent < ViewComponent::Base
  renders_one :header, lambda { |title: nil, subtitle: nil, hide_close_icon: false, **opts, &block|
    content_tag(:header, class: 'px-4 flex flex-col gap-2', **opts) do
      title_div = content_tag(:div, class: 'flex items-center justify-between gap-2') do
        if title
          title = content_tag(:h2, title, class: class_names('font-medium text-primary', drawer? ? 'text-lg' : ''))
        end
        unless hide_close_icon
          close_icon = render ButtonComponent.new(variant: 'icon', class: 'ml-auto', icon: 'x', tabindex: '-1', data: { action: 'dialog#close' })
        end
  ...
  renders_many :actions, lambda { |cancel_action: false, **button_opts|
  ...
  renders_many :sections, lambda { |title:, **disclosure_opts, &block|
  ...
  def initialize(variant: 'modal', auto_open: true, reload_on_close: false, width: 'md', frame: nil,
                 disable_frame: false, **opts)
  ...
  def wrapper_element(&)
    if disable_frame
      ...
      content_tag('turbo-frame', id: frame, &)
    end
  end
