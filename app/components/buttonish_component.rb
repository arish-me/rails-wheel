class ButtonishComponent < ViewComponent::Base
  ...
  def initialize(variant: :primary, size: :md, href: nil, text: nil, icon: nil, icon_position: :left,
                 full_width: false, frame: nil, **opts)
  ...
  def container_classes(_override_classes = nil)
    class_names(
      'font-medium whitespace-nowrap',
      merged_base_classes,
      full_width ? 'w-full justify-center' : nil,
      container_size_classes,
      size_data[:text_classes],
      variant_data[:container_classes]
    )
  end

  def container_size_classes
    icon_only? ? size_data[:icon_container_classes] : size_data[:container_classes]
  end
  ...
  def icon_classes
    class_names(
      variant_data[:icon_classes]
    )
  end

  def icon_only?
    variant.in?(%i[icon icon_inverse])
  end
  ...
  def merged_base_classes
    ...
    base_radius_classes = size_data[:radius_classes]
    ...
    base_classes << base_display_classes unless has_display_override

    base_classes << base_radius_classes unless has_radius_override
