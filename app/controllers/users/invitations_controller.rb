module Users
  class InvitationsController < Devise::InvitationsController
    ...
    def create
      ...
      if resource_invited
        ...
        if method(:after_invite_path_for).arity == 1
          ...
          respond_with resource, location: after_invite_path_for(resource, resource)
        end
      else
    ...
    def invite_resource(&)
      ...
      resource = resource_class.invite!(invite_params, current_inviter, &)

      return resource if resource.errors.present?
      ...
      if resource.persisted?
        ...
        update_attrs[:company_id] = current_inviter.company_id if current_inviter.company_id.present?

        resource.update_columns(update_attrs)
        # Assign default role for the company if company exists
        resource.assign_default_role if current_inviter.company_id.present?
      end
    ...
    def authenticate_inviter!
      redirect_to root_path, alert: "You don't have permission to invite users." unless current_user
      current_user
    end
