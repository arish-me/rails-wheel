namespace :subscriptions do
  ...
  task send_expiry_notifications: :environment do
    ...
    expiring_subscriptions = CompanySubscription.active.where("end_date <= ? AND end_date > ?", 7.days.from_now,
                                                              Time.current)
  ...
  task create_missing_trials: :environment do
    ...
    companies_without_subscriptions = Company.where.missing(:company_subscription)
